var MUB = {
    _uid : 0
    ,_minRoot : '/components/com_minify/?'
    ,checkRewrite : function () {
        ...
        $.ajax({
            ...
            ,success : function (data) {
                if (data === '1') {
                    MUB._minRoot = '/components/com_minify/';
                    $('span.minRoot').html('/components/com_minify/');
                } else
                    fail();                
            }
            ,error : fail
    ...
    ,init : function () {
        ...
        if (location.hash) {
            ...
            var bmUri = "administrator/components/com_minify/builder/bm.js";
            	//location.pathname.replace(/\/[^\/]*$/, '/bm.js').substr(1);
            $.ajax({
                url : '../?f=' + bmUri
                ,success : function (code) {
                    $('#bm')[0].href = code
                        .replace('%BUILDER_URL%', "http://localhost/Joomla1.5.8/administrator/components/com_minify/builder/")
                        .replace(/\n/g, ' ');
                }
                ,dataType : 'text'
            });
