...
class SEFTools
{
    ...
    function &getExtParams($option)
    {
        ...
        if (! isset($exts[$lang])) {
            $query = "SELECT `id`, `file`, `params` FROM `#__sefexts`";
            $db->setQuery($query);
            $exts[$lang] = $db->loadObjectList('file');
        }
        ...
        if (! isset($params[$option])) {
            $data = '';
            if (isset($exts[$lang][$option . '.xml'])) {
                $data = $exts[$lang][$option . '.xml']->params;
            }
    ...
    function GetSEFGlobalMeta() {
        return 'a79ac06f7f1d461c4a302a8bba2122c4'; // sef.global.meta
    }
