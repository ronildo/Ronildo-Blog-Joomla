...
function com_uninstall()
{
    ...
    if ($config->_complete_uninstall) {
    	...
    	$database->query();
            $database->setQuery("DROP TABLE IF EXISTS `#__comment_joomvertising`");
    	$database->query();
    ...
    $database->setQuery("DELETE FROM `#__plugins` WHERE `element`= 'joscomment';");
    $database->query();
    @unlink(JPATH_SITE."/plugins/content/joscomment.php");
    @unlink(JPATH_SITE."/plugins/content/joscomment.xml");
    $database->setQuery("DELETE FROM `#__plugins` WHERE `element`= 'josccleancache';");
    $database->query();
    @unlink(JPATH_SITE."/plugins/system/josccleancache.php");
    @unlink(JPATH_SITE."/plugins/system/josccleancache.xml");
