...
/*
 * Copyright Copyright (C) 2008-2009 Compojoom.com. All rights reserved.
        ...
        for ($i = 0, $n = count($rows); $i < $n; $i++) {
            ...
            $row->comment 	= isset($row->comment) ? $row->comment : ''; /* to avoid notice in import preview */
				...
				$row->comment = $post->censorText(JOSC_utils::filter($post->_item['comment']));
                                ...
                                if (JString::strlen($row->comment) > $max_length_word) {

                                    $row->comment = JString::substr($row->comment, 0, $max_length_word);
                                    $row->comment .= "...";
                                }
...
?>
<?php
    $link_edit = JRoute::_('index.php?option='.$option.'&task=edit&id='.$row->id);
?>
...
?>
				<img border="0" src="<?php echo JURI::root().'components/com_comment/joscomment/images/'.$notifyimg; ?>" title="<?php echo $notifytxt; ?>" alt="<?php echo $notifyalt; ?>" />
			</a>
		</td>                
  ...
  <form action="index.php" method="post" name="adminForm" id="adminForm">
    <table cellpadding="4" cellspacing="1" border="0" width="100%" class="adminform">
      ...
      <tr>
        ...
        <td>
          <input class="inputbox" type="text" name="title" value="<?php echo stripslashes($row->title);
      ...
      <tr>
        ...
        <td>
            <textarea class="inputbox" cols="50" rows="5" name="comment"><?php echo stripslashes($row->comment);
