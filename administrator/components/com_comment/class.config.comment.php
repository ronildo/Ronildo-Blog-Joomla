...
/*  params */    
    ...
    var $_akismet_key;
    var $_gravatar;
    var $_disable_additional_comments;
	...
	$database =& JFactory::getDBO();
        ...
        $this->_configFile = JPATH_SITE.DS.'administrator'.DS.'components'.DS.'com_comment'.DS.'defaultconfig.comment.php';
    ...
    function load()
    {
        ...
        $this->_akismet_use             = $params->def( '_akismet_use'          , $_akismet_use);
        $this->_akismet_key             = $params->def( '_akismet_key'          , $_akismet_key);
	$this->_gravatar		= $params->def( '_gravatar'		, $_gravatar);
	$this->_disable_additional_comments = $params->def('_disable_additional_comments', $_disable_additional_comments);
...
*/
	...
	$file = $this->_template_custom_path. DS . $template .DS.'index.html';
	...
	$file = $this->_template_custom_path. DS . $template . DS . 'css' .DS. $templateCSS;
	...
	$row 			= new JOSC_tabRow();
        $row->caption 	= _JOOMLACOMMENT_ADMIN_disable_additional_comments_CAPTION;
        $row->component = JOSC_library::input('params[_disable_additional_comments]', 'class="inputbox"', $this->_disable_additional_comments);
        $row->help 		= _JOOMLACOMMENT_ADMIN_disable_additional_comments_HELP;
        $rows->addRow($row);
		$row 			= new JOSC_tabRow();
    ...
    function postingPage()
    {
		...
		$rows->addRow($row);
		$row 			= new JOSC_tabRow();
        $row->caption 	= _JOOMLACOMMENT_ADMIN_gravatar_CAPTION;
        $row->component = JHTML::_('select.booleanlist',  'params[_gravatar]', 'class="inputbox"', $this->_gravatar);
        $row->help 		= _JOOMLACOMMENT_ADMIN_gravatar_HELP;
		$rows->addRow($row);
        $row = new JOSC_tabRow();
...
*/
    ...
    function emoticons_confightml($pack) {
        ...
        if (file_exists($file)) {
            ...
            require_once($file);
            require_once(JPATH_SITE.DS.'components'.DS.'com_comment'.DS.'joscomment'.DS.'comment.class.php');
...
//        $row->component = JOSC_library::usertypes('_captcha_usertypes', $this->_captcha_usertypes);
        ...
        $rows->addRow($row);
        $row            = new JOSC_tabRow();
        $row->caption   = _JOOMLACOMMENT_ADMIN_akismet_use;
        $row->component = JHTML::_('select.booleanlist', 'params[_akismet_use]', 'class="inputbox"', $this->_akismet_use);
        $row->help 		= _JOOMLACOMMENT_ADMIN_akismet_HELP;
        $rows->addRow($row);
        $row            = new JOSC_tabRow();
        $row->caption   = _JOOMLACOMMENT_ADMIN_akismet_key;
        $row->component = JOSC_library::input('params[_akismet_key]', 'class="inputbox"', $this->_akismet_key);
        $row->help 		= _JOOMLACOMMENT_ADMIN_akismet_key_HELP;
        $rows->addRow($row);
        $row 			= new JOSC_tabRow();
...
<?php
	...
	$file = $this->_template_custom_path . DS . $this->_template_custom . DS . "index.html";
	if ( $fp = fopen( $file, 'r' ) ) {
	...
	$file = $this->_template_custom_path . DS . $this->_template_custom . DS . "css" . DS . $this->_template_custom_css;
