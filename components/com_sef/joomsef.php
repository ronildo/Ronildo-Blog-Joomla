...
class JoomSEF
{
    ...
    function build(&$uri)
    {
        ...
        if (!is_null($option)) {
            ...
            // Check the variable filter test
            if( !JoomSEF::_varFilterTest($uri) ) {
                // Don't SEF this URL
                $uri = JoomSEF::_createUri($uri);
                JoomSEF::_endSef($prevLang);
                return;
        ...
        if (SEFTools::JoomFishInstalled() && ($sefConfig->langPlacement == _COM_SEF_LANG_DOMAIN)) {
            ...
            foreach ($sefConfig->jfSubDomains as $langCode => $domain) {
                if ($host == $domain) {
                    ...
                    JRequest::setVar('lang', $langCode);
                    $config =& JFactory::getConfig();
                    $config->set('joomsef.domain_lang', $langCode);
    ...
    function _getCustomMenuTitle($option)
    {
        ...
        return $params->get('customMenuTitle', '');
        /*
        $db =& JFactory::getDBO();
        $sefConfig =& SEFConfig::getConfig();
    ...
    function _sefGetLocation(&$uri, &$title, $task = null, $limit = null, $limitstart = null, $lang = null, $nonSefVars = null, $ignoreSefVars = null, $metadata = null, $priority = null, $pageHandled = false)
    {
        ...
        if( is_null($priority) ) {
            $priority = self::_getPriorityDefault($uri);
        }
        ...
        if (!$sefConfig->useCache || !$realloc) {
            $query = "SELECT `sefurl` FROM `#__sefurls` WHERE `origurl` = '" . addslashes(html_entity_decode(urldecode($url))) . "'" . $where;
            $db->setQuery($query);
        ...
        // URL not found, so lets create it
            ...
            $realloc = self::_checkRow($row, $ignoreSource, @$Itemid, $url, $metadata, $temploc, $priority, $uri->getVar('option'));
    ...
    function _checkRow(&$row, $ignoreSource, $Itemid, $url, &$metadata, $temploc, $priority, $option)
    {
        ...
        if( ($row != false) && !is_null($row) ) {
            ...
            if( !$numberDuplicates ) {
                ...
                self::_saveNewURL($Itemid, $metadata, $priority, $temploc, $url);
                $realloc = $temploc;
            }
        }
        // URL not found
