...
/*
 * Copyright Copyright (C) 2010 Daniel Dimitrov (compojoom.com). All rights reserved.
...
//        $this->_censorship_words = explode(',', $config->_censorship_words);
        ...
        $this->loadLanguage($absolutePath, $this->_language);
...
class JOSC_visual extends JOSC_properties {
	...
	{
	    return '<div id="poweredby" align="center" class="small"><a target="_blank" href="http://compojoom.com" title="Joomla components"> Joomla components </a> by Compojoom</div>';
	}
...
class JOSC_post extends JOSC_support {
    ...
    function voting_cell($mode, $num, $id) {
        return "<li><div id='$mode$id' class='voting_$mode' onclick='JOSC_voting($id,\"$mode\")'>$num</div></li>";
    }
...
/*        $result = "<table class='postcontainer' id='post$id' width='100%' cellpadding='0' cellspacing='0' style='padding-left: $wrapnum;'>";
	...
	if(strcmp($id,'')) {
	    $link = JRoute::_('index.php?option=com_comprofiler&task=userProfile&user='.$id);
            return "<a class='username' href='$link'>".$s."</a>";
	} else {
            return "<span class='username'>".$s."</span>";
	}
    }
...
// Remove everything but the newline charachter
	...
	public static function do_bbcode_quote($action, $attributes, $content, $params, $node_object) {
		...
		if (isset($attributes['default'])) {
            $quote = "<div class='quote'><div class='genmed'><b>" . $attributes['default'] . ' ' . _JOOMLACOMMENT_UBB_WROTE . "</b></div><div class='quotebody'>".$content.'</div></div>';
		} else {
            $quote = "<div class='quote'><div class='genmed'><b>" . _JOOMLACOMMENT_UBB_QUOTE . "</b></div><div class='quotebody'>".$content.'</div></div>';
		}
		return $quote;
	}

	public static function do_bbcode_code($action, $attributes, $content, $params, $node_object) {
		$quote = "<div class='code'><div class='genmed'><b>" . _JOOMLACOMMENT_UBB_CODE . "</b></div><div class='quotebody'>".$content.'</div></div>';
		return $quote;
	}
	...
	public static function do_bbcode_color ($action, $attributes, $content, $params, $node_object) {
		...
		if (isset ($attributes['default'])) {
			if ($action == 'validate') {
				return JOSC_ubbcode::validate($attributes['default']);
			}
...
class JOSC_form extends JOSC_support {
    ...
    function disableAdditionalComments()
    {
	return '<div class="nomore">' . _JOOMLACOMMENT_DISABLEADDITIONALCOMMENTS . '</div>';
    }

    function readOnly($username)
    {
        if ($username) return 'disabled="disabled"';
        else return '';
    }
	...
	function form_htmlCode() {
	    ...
	    $html = str_replace('{_COMMENT}', _JOOMLACOMMENT_COMMENTS_1, $html);
	    $html = str_replace('{_YOUR_CONTACT_DETAILS}', _YOUR_CONTACT_DETAILS, $html);
	    $html = str_replace('{_SECURITY}', _SECURITY, $html);
	    $html = str_replace('{_MESSAGE}', _MESSAGE, $html);
	    ...
	    if ($this->_gravatar) {
		$html = str_replace('{gravatar}', _JOOMLACOMMENT_GRAVATAR_ENABLED , $html);
	    } else {
...
class JOSC_search extends JOSC_support {
    ...
    {
        if ($name == '') {
            $name = JOSC_utils::filter($this->encodeData_Charset(_JOOMLACOMMENT_ANONYMOUS));
        }
        return $name;
    }
...
//    }
    ...
    function trimResult($html, $word, $size)
    {
        ...
        if ($p == 0) return JString::substr($html, 0, $size);
...
//        $address = 'javascript:JOSC_goToPost(' . $item['contentid'] . ',' . $item['id'] . ')';
    ...
    {
        $result = '';
        if (!JString::strpos($this->_keyword, ' ')) return $this->terms($list, "LIKE '%$this->_keyword%'");
        $words = split(' ', $this->_keyword);
        foreach($words as $item) {
            if ($item != '') {
                if ($result != '') $result .= ' OR ';
