...
//	}
	...
	if (($_maxlength_line > -1)&&($_maxlength_line!=999999)) {
	    return JOSC_utils::utf8_wordwrap($text, $_maxlength_line, $char, true);
	} else {
	    return $text;
	}
    }
    /*
     * This function is used to only wrap a word.
     * It first splits the string on every line, then looks for the word length
     *
	...
	$word_count = 0;
	$pos = 0;
	$str_len = JString::strlen($str);
	$str .= ' <';
	$open_tags = array ();

	while ($word_count < $no_words_ret && $pos < $str_len) {
	    $pos = min(JString::strpos($str, ' ', $pos), JString::strpos($str, '<', $pos));
		...
		if ($str[$pos + 1] == '/') {
		    ...
		    $sub = JString::substr($str, $pos + 1, min(JString::strpos($str, ' ', $pos), JString::strpos($str, '>', $pos)) - $pos - 1);
		    if (in_array($sub, $tags)) {
			array_push($open_tags, $sub);
		    }
		}
		$pos = JString::strpos($str, '>', $pos) + 1;
	    } else {
		$pos++;
		$word_count++;
	    }
